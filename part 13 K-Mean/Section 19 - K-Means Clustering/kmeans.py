...
3. Assign each data point to the closest centroid -> that froms K clusters
4. Computer and place the new centroid of each cluster
5. Resassign each data point to the new closest centroid
...
plt.xlabel('Number of Clusters')  # choose the right number of cluster, this we take 5
plt.ylabel('WCSS')
plt.show()
...
"""
we use 5 groups to label and group our customers. 
"""
...
plt.legend()
plt.show()
### predict new_data = np.array([[new_point_feature_1, new_point_feature_2], ...])

new_data = np.array([[20, 75]]) # annual income 20, score 75
predicted_clusters = kmeans.predict(new_data)
# prediction shows it is 3 in cyan